...
# ----------------------------------------------------------------------
# Lógica de BIN y formateo (idéntica a la original)
...
def load_bin_database(csv_path: str) -> Dict[str, Dict[str, str]]:
    """
    Carga la base de datos de BINs desde el archivo CSV especificado.
    Columnas esperadas: bin, brand, tipo, nivel, Banco, país
    """
    ...
    try:
        with open(csv_path, "r", encoding="utf-8-sig", newline="") as f:
            ...
            if "card data" not in (reader.fieldnames or []):
                raise ValueError(f"El archivo {csv_path} no contiene la columna 'card data'")
            for row in reader:
...
# ----------------------------------------------------------------------
# Envío de un solo mensaje (con reintento simple)
...
async def main():
    ...
    logger.info(f"Archivo de tarjetas (card data): {CSV_CARDS_FILE}")
