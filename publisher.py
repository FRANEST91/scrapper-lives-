...
# ----------------------------------------------------------------------
# Base de datos compartida (misma que usa el verificador para insertar)
...
def _load_bin_database(csv_path: str) -> Dict[str, Dict[str, str]]:
    ...
    try:
        ...
        logger.info("Base BIN cargada: %d entradas desde %s", len(bin_db), csv_path)
...
def _format_card_message(card_data: str, bin_database: Dict[str, Dict[str, str]]) -> Optional[str]:
...
def publish_pending_cards(db_path: str = DB_PATH, delay: float = 1.5) -> int:
    ...
    Args:
        db_path: Ruta a la base SQLite compartida
        delay: Segundos entre cada mensaje
    
    Returns:
        Número de tarjetas publicadas con éxito
    """
    conn = _init_db(db_path)
    
    # Leer pendientes no publicadas
    rows = conn.execute(
    ...
    # Cargar base BIN para formatear
